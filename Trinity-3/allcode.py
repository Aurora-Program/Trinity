class Transcender:
    ...
    def compute_fractal(self, fractal_tensor):
        """
        Procesa un FractalTensor de manera recursiva y jerárquica:
        - nivel_27: sintetiza cada trío consecutivo para formar nivel_9
        - nivel_9: sintetiza cada trío consecutivo para formar nivel_3
        - nivel_3: sintetiza los tres resultados de nivel_9 para obtener el vector final
        Devuelve un diccionario con los resultados de cada nivel y la jerarquía de síntesis.
        """
        resultados = {}
...
class Evolver:
    def relate_between_tensors(self, tensorA, tensorB, nivel='nivel_3', distance_metric='hamming'):
        """
        Analiza la relación entre todos los vectores de un nivel específico de dos tensores diferentes.
        Devuelve la matriz de distancias y agrupamientos.
        """
        vectores_A = getattr(tensorA, nivel, [])
        vectores_B = getattr(tensorB, nivel, [])
        relaciones = []
        for i, va in enumerate(vectores_A):
            for j, vb in enumerate(vectores_B):
                if distance_metric == 'hamming':
                    dist = sum((a != b) if (a is not None and b is not None) else 0 for a, b in zip(va, vb))
                elif distance_metric == 'euclidean':
                    dist = sum(((a-b)**2) if (a is not None and b is not None) else 0 for a, b in zip(va, vb)) ** 0.5
                else:
                    dist = None
                relaciones.append({'A_idx': i, 'B_idx': j, 'dist': dist})
        return relaciones

class Evolver:
    def __init__(self):
        # El Evolver ahora necesita su propio Transcender para la meta-computación.
        self.base_transcender = Transcender()
        self.meta_transcender = Transcender() # Para operar sobre los MetaM
        self.axioms = []


    def compute_archetypes(self, *tensors, nivel='nivel_3'):
        """
        Descubre arquetipos después de la síntesis de vectores.
        Recibe N tensores fractales, toma el resultado sintetizado final de cada uno (por ejemplo, el último MetaM de cada tensor),
        y calcula el arquetipo global usando el meta_transcender.
        """
        print(f"\n[Arquetipos] Descubriendo arquetipo global tras síntesis en {nivel}...")
        # Extraer el último MetaM de cada tensor sintetizado
        metaMs = []
        for ft in tensors:
            results = self.base_transcender.compute_fractal(ft)
            metaMs_nivel = [res['MetaM'] for res in results[nivel] if res and 'MetaM' in res]
            if metaMs_nivel:
                metaMs.append(metaMs_nivel[-1])  # Solo el último vector sintetizado
        if len(metaMs) < 3:
            print("Advertencia: Se requieren al menos 3 tensores para descubrir arquetipo.")
            return {'error': 'No hay suficientes tensores para arquetipo.'}
        # Calcular arquetipo global
        if any(m is None or any(x is None for x in m) for m in metaMs):
            print("Advertencia: Algún MetaM contiene None. No se puede calcular arquetipo.")
            return {'error': 'MetaM incompleto.'}
        arquetipo = self.meta_transcender.compute(*metaMs[:3])  # Solo los 3 primeros si hay más
        print(f"Arquetipo global descubierto: {arquetipo}")
        return arquetipo
    ...
    # El método simple analyze_dynamics ha sido eliminado. Usar analyze_dynamics_adaptive para toda dinámica temporal.
    """
    ...
    # El método simple relate_vectors ha sido eliminado. Usar relate_vectors (antes relate_vectors_adaptive) para toda operación de relator.
    ...
    def relate_vectors(self, vectors, context_id=None, distance_metric='auto', affinity_threshold=None):
        """
        Relator adaptativo: detecta clústeres, outliers y ajusta el umbral de afinidad según la dispersión.
        Solo permite la emergencia de un nuevo vector superior si todos los vectores tienen información (no None) en todas las dimensiones.
        ...
        Devuelve dict con clústeres, outliers, matriz de distancias y, si corresponde, el vector superior emergente.
        """
        ...
        # Emergencia de vector superior solo si todos los vectores tienen información en TODAS las dimensiones
        all_full = all(
            isinstance(v, (list, tuple)) and len(v) == 3 and all(x is not None for x in v)
            for v in vectors
        )
        if all_full:
            emergent_vector = []
...
class Extender:
    ...
    def rebuild(self, ms, metam, ss):
        """
        Reconstruye la parte faltante de Ss usando reglas de progresión.
        
        Args:
            ms: Meta-estado (no usado en progresiones lineales)
            metam: Meta-meta-estado (no usado en progresiones lineales)  
            ss: Lista [delta_1, delta_delta, None] con hueco a rellenar
            
        Returns:
            Lista [delta_1, delta_delta, delta_2] completa
        """
        try:
            # Validar entrada
            ...
            # 🔍 REGLA LINEAL: Δ₂ = Δ₁ + ΔΔ (progresión aritmética con aceleración)
